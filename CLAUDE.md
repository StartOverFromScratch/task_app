...
**実際の開発環境は Docker（`docker-compose.dev.yml`）を使用する。**
ローカル venv はテスト実行・マイグレーション・パッケージ追加時のみ使用する。
...
# requirements.txt 変更後は再ビルドが必要
docker compose -f docker-compose.yml -f docker-compose.dev.yml build backend
```

**環境変数:** `.env.dev` に `CORS_ORIGINS`・`NUXT_PUBLIC_API_BASE`・`NUXT_API_BASE_SERVER`・`VAPID_*` を設定（`.env.example` 参照）。

---

### ローカル venv（テスト・マイグレーション専用）
...
Docker 経由で起動するため通常は直接操作しない。型チェック・lint のみローカルで実行する。パッケージマネージャーは **npm**。
