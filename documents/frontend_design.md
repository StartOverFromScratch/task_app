...
作成日: 2026-02-26 | バージョン: 1.7 | 最終更新: 2026-04-11
...
#### PC（lg以上）：2カラムレイアウト（v1.7更新）

左カラムがThings型ナビゲーションに変更。ナビ項目を選択すると右カラムにタスクリストを表示し、タスクを選択すると右カラムが詳細に切り替わる。
...
│  │                  │  │  支払い申請           [編集] [削除]      │ │
│  │ ★ 今日・明日    │  │  ────────────────────────────────────── │ │
│  │   （選択中）     │  │  [execution]  未着手  must  期限:04/09  │ │
│  │                  │  │                                          │ │
│  │ ≡ 全て          │  │  完了条件                                │ │
│  │                  │  │  支払い承認完了                          │ │
│  └──────────────────┘  └──────────────────────────────────────────┘ │
...
| `TaskStatusBadge.vue` | ステータスバッジ（詳細パネルで表示） |
| `TaskTypeBadge.vue` | タスクタイプバッジ |
...
| `all` | 全て | i-lucide-layers | フィルタなし（全ステータス・全期限） |
...
| `staleCount` | `number` | 放置タスク件数（右端に表示） |
| `carryoverCount` | `number` | 繰り越し候補件数（右端に表示） |
